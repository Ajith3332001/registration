...
const Register = () => {
  ...
  const sendOtp = async () => {
    setLoading(true);
    try {
      const response = await axios.post('127.0.0.1:8888/mail/send-otp', {
        email: user.email,
      });
      setLoading(false);
      setIsOtpSent(true);
  ...
  const verifyOtp = async () => {
    ...
    try {
      const response = await axios.post(
        '127.0.0.1:8888/mail/verify-otp',
        {
          email: user.email,
          otp,
        }
  ...
  const sendPhoneOtp = async () => {
    ...
    try {
      const response = await axios.post(
        'http://127.0.0.1:8888/mobile/send-otp',
        {
          phone: '+91' + user.phone,
        }
      );
...
const verifyPhoneOtp = async () => {
  ...
  try {
    const response = await axios.post(
      'http://127.0.0.1:8888/mobile/verify-otp',
      {
        phone: '+91' + user.phone,
        otp,
      }
  ...
  const verifyAadhar = async () => {
    ...
    try {
      const response = await axios.post(
        'http://127.0.0.1:8888/aadhar/verify-aadhar',
        {
          aadhar: user.aadhar,
        }
      );
  ...
  const verifyBank = async () => {
    ...
    try {
      const response = await axios.post(
        '127.0.0.1:8888/bank/verify-bank',
        {
          bank: user.bank,
          ifsc: user.ifsc,
        }
      ...
      try {
        const result = await axios.get(
          `127.0.0.1:8888/bank/verify-bank?id=${id}`
        );
  ...
  const getAddress = async () => {
    ...
    try {
      const response = await axios.get(
        `127.0.0.1/pincode/get-address?pincode=${user.pincode}`
      );
